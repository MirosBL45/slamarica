...
interface Props {
  ...
  setLocalPools: React.Dispatch<React.SetStateAction<Record<BudgetPoolType, number>>>;
}

const SettingsBudget = observer(({ month, activeType, localPools, setLocalPools }: Props) => {
      ...
      <Card className={styles.card}>
        ...
        <div className={styles.list}>
          {Object.entries(localPools).map(([type, value]) => (
            <div
              key={type}
              className={`${styles.row} ${activeType === type ? styles.activeRow : ""}`}
            >
              ...
              <div className={styles.controls}>
                <Slider
                  ...
                  className={`${styles.slider} ${styles[type as BudgetPoolType]}`}
                />
        ...
        <div className={styles.footer}>
          ...
          {!isLocked && (
            ...
            //   Sačuvaj
            // </Button>
            <ActionButton disabled={total !== 100} onClick={handleSave}>
              Sačuvaj
            </ActionButton>
          )}
        </div>
